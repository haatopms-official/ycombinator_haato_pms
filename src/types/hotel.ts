...
export interface Booking {
  ...
  /** Final cash price for this booking, entered by admin. */
  price?: number;
  ...
  /**
   * When true, the guest is expected as a LATE (night) check-in — the
   * effective deadline shifts to 23:59 instead of 11:59. The missed-
   * check-in critical alert therefore only starts pulsing one hour
   * before that, at 22:59 on the check-in date.
   */
  checkInLateNight?: boolean;
  /**
  ...
  /**
   * How the booking was created. 'offline' is auto-assigned whenever a
   * booking is created manually by admin from inside
   * the panel. 'online' is reserved for bookings that arrive from any
   * external/online channel. The system sets this automatically — the UI
