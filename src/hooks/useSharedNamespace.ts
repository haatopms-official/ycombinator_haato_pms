...
// Module-level cache shared by every useSharedNamespace instance in the same
// window. Without this, two components (e.g. BookingDialog and
// GuestDetailsWindow) each keep their own useState copy and only see each
// other's writes on the next 2s cloud poll, so edits made in one panel do
// not appear in the other until seconds later. The cache + custom event
// make writes visible to every mounted instance immediately.
 ...
 * identical for the admin panel everywhere.
 */
export function useSharedNamespace(key: HotelStateKey, eventName: string) {
  ...
  // Sync-across-instances: listen for the custom event and refresh from the
  // shared cache so sibling hooks pick up local writes without waiting for
  // the 2s cloud poll.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const pull = async () => {
      try {
        ...
        const next = (row.stateData as RecordMap) || {};
        memCache[key] = next;
        setMap(next);
