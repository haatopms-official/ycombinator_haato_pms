...
export function HotelEditBookingModal({ open, onClose, onSave, onUpdate, onDelete, roomNumber, booking }: EditBookingModalProps) {
  ...
  // Serialized snapshot of every editable field. Used to detect unsaved changes
  // when the user tries to close the modal.
  const currentSnapshot = useMemo(
    () => JSON.stringify({
      firstName, lastName, middleName,
      guestPhone, guestWhatsapp, guestTelegram, guestInstagram, guestEmail,
      guestCount, notes, checkIn, checkOut, status, price,
    }),
    [firstName, lastName, middleName, guestPhone, guestWhatsapp, guestTelegram, guestInstagram, guestEmail, guestCount, notes, checkIn, checkOut, status, price],
  );
  latestSnapshotRef.current = currentSnapshot;

  // Re-baseline shortly after opening, once fields and auto-calculated values
  // have hydrated into the form.
  useEffect(() => {
    if (!open) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && requestClose()}>
      <DialogContent className="sm:max-w-[860px] p-0 overflow-hidden border-0 bg-transparent shadow-none [&>button.absolute]:hidden">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-3 border-t border-border/60 bg-background/60 px-7 py-4 backdrop-blur">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={requestClose} className="rounded-xl">
                {t('cancel')}
              </Button>
              <Button
                size="sm"
      ...
      <UnsavedCloseWarning
        ...
        saveDisabled={!guestName || !checkIn || !checkOut}
      />
    </Dialog>
  );
}
