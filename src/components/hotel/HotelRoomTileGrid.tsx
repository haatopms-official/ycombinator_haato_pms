...
interface RoomTileGridProps {
  ...
  /**
   * Open the SAME BookingDialog the main grid uses, pre-filled with a specific
   * booking. Ensures the second grid's filling box is 1:1 identical with the
   * main grid's (same fields, same handlers, same context).
   */
...
export function HotelRoomTileGrid({ rooms, bookings, activeFilter, selectedDate, onEditRoom, onShowOnGrid, onEditBooking }: RoomTileGridProps) {
  const { lang, t } = useI18n();
  const { categories } = useHotelGrid();

  // Dropdown selection per room (bookingId). Resets implicitly when room list changes.
  ...
  const renderTile = ({ room, roomBookings }: { room: Room; roomBookings: Booking[] }) => {
    ...
    return (
      <div
        ...
        onClick={() => {
          // Clicking anywhere on the tile (other than the explicit
          // "View on grid" button) opens the SAME BookingDialog the main
          // grid uses, pre-filled with this booking. Only the dedicated
          // "View on grid" button redirects the user to the timeline.
      ...
      >
        ...
        <div className="flex items-start justify-between mb-1.5 pr-6">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              // Prefer editing the exact selected booking so the dialog is
              // pre-filled identically to the main grid's editor.
              if (booking && onEditBooking) onEditBooking(booking.id);
              else onEditRoom(room.number);
            }}
        ...
        {roomBookings.length >= 1 && (
          <div className="mb-2" onClick={(e) => e.stopPropagation()}>
            <Select
              ...
              onOpenChange={(open) => {
                // Closing the guest dropdown must also stop any pulsating
                // focus-glow the operator triggered from this tile, so the
                // next click on the main grid is not swallowed by the
                // "click-anywhere-cancels" listener.
