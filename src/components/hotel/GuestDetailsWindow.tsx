  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span className={`flex-1 truncate font-semibold ${!value ? 'text-muted-foreground/60 font-normal' : ''}`}>
          {current ? current.label : (placeholder || 'Select gender')}
        </span>
        <svg className={`h-4 w-4 shrink-0 text-muted-foreground transition ${open ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m6 9 6 6 6-6"/></svg>
      </button>
      {open && (
        <div className="absolute left-0 right-0 top-full z-[95] mt-2 overflow-hidden rounded-2xl border border-border/60 bg-popover p-1.5 shadow-2xl shadow-primary/15">
          {GENDER_OPTIONS.map((o) => {
            ...
            return (
              ...
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-md bg-primary/10 text-primary text-base">{o.icon}</span>
...
export function GuestDetailsWindow({ open, onClose, guest }: GuestDetailsWindowProps) {
  ...
  // the admin panel and modal (Anketa). We use the
                    ...
                    {PASSPORT_FIELDS.map((f, idx) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`rounded-xl border border-border bg-background/60 p-3 transition-colors hover:border-primary/40 ${(f.span as number) === 2 ? 'sm:col-span-2' : ''}`}
                        >
